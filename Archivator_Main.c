...
int Create(char* path) {
    /*
    Function: создание архива по указанному адресу, передаваемому в функцию
...
int main() {
    ...
    while (1) {
        ...
        if (isOutput) {
            ...
            Output(isVerbose, currentPath, defaultPath, mode);
        ...
        if (isCreate) {
            ...
            /*---DEBUG---*/
            if (Create(currentPath)) /*something*/;             //error
